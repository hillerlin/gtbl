...
class Verify {
    ...
    public function entry($id = '') {
        ...
        if($this->useZh){ // 中文验证码
            ...
            $algorithm = array('+', '-');
            $formula[1] = $algorithm[array_rand($algorithm)];
            $formula[0] = mt_rand(0, 9);
            if ($formula[1] == '-') {
               $formula[2] = mt_rand(0, $code[0]); 
               $code = $formula[0] - $formula[2];
            } else {
                $formula[2] = mt_rand(0, 9); 
                $code = $formula[0] + $formula[2];
            }
            $formula[3] = '=';
            for ($i = 0; $i<$this->length; $i++) {
                $code[$i] = $this->codeSet[mt_rand(0, 9)];
                $codeNX  += mt_rand($this->fontSize*1.2, $this->fontSize*1.6);
                imagettftext($this->_image, $this->fontSize, 0, $codeNX, $this->fontSize*1.6, $this->_color, $this->fontttf, $formula[$i]);
            }
        } else {
            for ($i = 0; $i<$this->length; $i++) {
